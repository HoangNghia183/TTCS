            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 mb-12">
                ...
                <div className="flex flex-col gap-5">
                    {(product.originalPrice && product.originalPrice > product.price) && (
                        <span className="badge-sale self-start">−{discount}%</span>
                    )}
                    ...
                    {Boolean(product.specifications?.["Giống"] || product.specifications?.["Tuổi"]) && (
                        <p className="text-muted-foreground text-sm">
                            {product.specifications?.["Giống"] && (
                                <>📖 Tác giả/NXB: <strong>{product.specifications["Giống"]}</strong></>
                            )}
                            {product.specifications?.["Tuổi"] && (
                                <> ⏳ Năm XB: <strong>{product.specifications["Tuổi"]}</strong></>
                            )}
                        </p>
                    )}
                    ...
                    <div className="flex items-center gap-2">
                        <div className="flex">
                            {[1, 2, 3, 4, 5].map((s) => (
                                <svg key={s} className={`w-5 h-5 ${s <= Math.round(product.averageRating) ? "text-amber-400" : "text-gray-200"}`} fill="currentColor" viewBox="0 0 20 20">
